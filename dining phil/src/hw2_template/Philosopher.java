...
public class Philosopher extends Thread {
	...
	public static void main(String args[]) {
		...
		for (int i=0; i < N; i++)
		{
			...
			oldies[i].start();
			//commented out this so the random sleep mechanism can be implemented in threads
			//because this for loop starts a thread with 5sec intervals
			/*
			try {
				sleep(5000);
			} catch (InterruptedException e) {
	...
	@Override
	public void run() {
		//each thread will sleep randomly 1-10s
		try {
		...
		//after sleep put plate on table
		this.table.PutPlate_GUI(this.id);

		//BARRIER
		//this algorithm is the same as the one in shown in recitation(but with 5 threads):
		//semaphore a,b,c = 0;
		// A->up b,up c,down a, down a
		// B->up a,up c,down b,down b
		// C->up a,up b,down c,down c
		try{
			for (int j = 0; j < N; j++) { //increase every thread's barrier semaphore by 1, except this current thread
				if (j != id){
					barriers[j].release();
				}
			}
			//acquire 4 times to guarantee current thread is in sync with others
			this.barriers[id].acquire();
			this.barriers[id].acquire();
			this.barriers[id].acquire();
			this.barriers[id].acquire();
		} catch (InterruptedException e) {
		...
		while(true){
			...
			//change state to think, make gui black&white, think for a random time 0-10s
			this.state[id] = THINKING;
			this.table.StopEating_GUI(id);
			try {
			...
			//take fork, same algorithm as in slides, nothing changed, only gui elements are added
			try {
				mutex.acquire();
				this.state[id] = HUNGRY;
				this.table.Hungry_GUI(id); //make gui hungry
				if(this.state[id] == HUNGRY && this.state[left] != EATING && this.state[right] != EATING){
					this.table.ForkTake_GUI(this.id); //make gui take fork
					this.sem[id].release();
				}
				mutex.release();
				this.sem[id].acquire();
			...
			//eat
			this.state[id] = EATING;
			this.table.ForkTake_GUI(this.id);
			this.table.Eating_GUI(this.id);


			try {//put fork, same algorithm as in slides, nothing changed, only gui elements are added
				mutex.acquire();
				this.state[id] = THINKING;
				this.table.ForkPut_GUI(id); //make gui put fork
				this.table.StopEating_GUI(id); // make gui stop eating
				if(this.state[left] == HUNGRY && this.state[(left+N-1)%N] != EATING && this.state[id] != EATING){
					this.table.ForkPut_GUI(this.id);
					this.state[left] = EATING;
					this.sem[left].release();
				}if(this.state[right] == HUNGRY && this.state[id] != EATING && this.state[(right+1)%N] != EATING){
					this.state[right] = EATING;
					this.sem[right].release();
				}
				mutex.release();
			} catch (InterruptedException e) {
				e.printStackTrace();
